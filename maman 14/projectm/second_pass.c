...
void write_output_entry(FILE *fp)
{
    ...
    while(label)
    {
        if(label -> entry)
        {
            base64_address = convert_to_base_64(label -> address);
            fprintf(fp, "%s\t%s\n", label -> name, base64_address);
            free(base64_address);
        }
        label = label -> next;
    }
...
void write_output_extern(FILE *fp)
{
    ...
    do
    {
        base64_address = convert_to_base_64(node -> address);
        fprintf(fp, "%s\t%s\n", node -> name, base64_address); /* Printing to file */
        free(base64_address);
        node = node -> next;
    } while(node != ext_list);
    fclose(fp);
